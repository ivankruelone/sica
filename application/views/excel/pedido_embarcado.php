...
$objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue('C1', 'Pedido Embarcado')
            ->setCellValue('D1', 'ID Pedido:')
            ->setCellValue('E1', $row->id)
            ;

$objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue('A2', 'Surtido a: '.$row->numsuc.' - '.$row->sucursal)
            ;
...
// Rename sheet
$objPHPExcel->getActiveSheet()->setTitle('Pedido Embarcado '.$row->id);
...
header('Content-Disposition: attachment;filename="pedido_embarcado_'.$row->id.'_'.date('YmsHsi').'.xlsx"');
