...
$('#nuevo_proveedor_form').submit(function(event)
{
    ...
    var url = "<?php echo site_url();?>/proveedores/submit_nuevo_proveedor";
    
    var variables = {
        rfc: $('#rfc').attr('value'),
        npro: $('#npro').attr('value'),
    ...
    $.post( url, variables, function(data) {
        if(data > 0){
            window.location = "<?php echo site_url();?>/proveedores/catalogo/";
        }else{
